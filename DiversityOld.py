...
Initial Solutions
'''
lowerLimit=0;upperLimit=2;collectionSize=100;vec_dim=N
...
def run(pool,mutP,selectionLength):
    
    
    children=[]
    uniqC = []
    for i in range(len(pool)):
        pairs = pickXOver(pool,2) #selecting two parents for x-over  
    ...
    if len(uniqC)>selectionLength:            
        maxmin = MaxMin(uniqC,selectionLength,dType)
...
if __name__=="__main__":
    ...
    for i in range(trials):    
        for j in range(generations):
            ...
            vecSet = run(pool,mutP,selectionLength)
